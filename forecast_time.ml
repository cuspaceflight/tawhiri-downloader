...
let () =
  [%test_eq: string]
    (to_string_yyyymmddhh (Date.of_string "1994-03-14", `h06))
    "1994031406";
  [%test_eq: string]
    (to_string_yyyymmdd_slash_hh (Date.of_string "1994-03-14", `h06))
    "19940314/06"
;;
...
let expect_next_release () =
  ...
  let hour =
    Time.Ofday.to_parts ofday |> (fun x -> x.Time.Span.Parts.hr) |> fun x -> x - (x mod 6)
  in
  let hour =
    match hour with
    | 0 -> `h00
